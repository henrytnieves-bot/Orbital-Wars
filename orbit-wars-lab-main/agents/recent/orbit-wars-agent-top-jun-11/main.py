...
def plan_lite_waves(
    *,
    ...
    Builds candidate fractions of safe_drain (0.5, 0.75, 1.0) per (source, target)
    shortlist pair, scores them with the competitive flow diff, and greedily fires
    the best wave per target up to max_waves_per_turn.
    """
    P = obs.P
    device = obs.device
    dtype = obs.ships.dtype
    ...
    # Multi-size wave planning: 3 candidate fractions of safe_drain (0.5, 0.75, 1.0)
    # Shape: [S, T, G] where G = 3
    G = 3
    fractions = torch.tensor([0.5, 0.75, 1.0], device=device, dtype=dtype).view(1, 1, G)
    sizes = (drain.view(S, 1, 1) * fractions).expand(S, T, G).floor()            # [S, T, G]

    # Reachability precheck
    active = reachable_mask(
        movement, source_idx=source_idx, target_idx=target_idx,
        fleet_sizes=sizes, eta_cap=eta_cap,
    )                                                                            # [S, T, G]
    aim = intercept_angle(
        movement,
    ...
    angle = aim["angle"]                                                         # [S, T, G]
    eta = aim["eta"]                                                             # [S, T, G]
    viable = aim["viable"] & (eta <= eta_cap.view(1, T, 1))

    if K > 0:
        k_arr = (eta.clamp(min=1.0, max=float(K)).ceil().long() - 1).clamp(0, K - 1)  # [S, T, G]
        floor_expanded = floor.view(1, T, 1, K).expand(S, T, G, K)
        floor_at_arr = floor_expanded.gather(-1, k_arr.unsqueeze(-1)).squeeze(-1) # [S, T, G]
    else:
        floor_at_arr = torch.ones(S, T, G, dtype=dtype, device=device)
    clears_floor = sizes >= floor_at_arr                                         # [S, T, G]
    ...
    # --- pack S * T * G candidates; contributor axis L = 1
    L = 1
    C = S * T * G
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        mem = self.memory
        is_step_0 = bool((obs_tensors["step"] == 0).all())
        if is_step_0:
            mem.reset()
            player_count = 2
        else:
        ...
        row = run_turn(
            ...
            player_count=int(player_count), memory=mem,
        )
