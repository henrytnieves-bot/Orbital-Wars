"""
main.py — Orbit Wars: Hybrid v4 + Sun Avoidance + Adaptive Horizon

Base : Producer Hybrid v4 (tuned 4P FFA preset + leader-guard score bonus)
Merged-in changes (marked , from the "Improved Producer" notebook):
  1. Sun avoidance   — sun_cross_mask_tensor ANDed into the viable mask;
                         no fleet is ever fired through the sun.
  2.  Adaptive horizon — 2P early game (step < 30) uses horizon=24,
                          late game reverts to 18. 4P never extends
                          (stays at the tuned CONFIG_4P value of 13).

Everything else (shortlists, ROI threshold, regroup, FFA bonuses) is the
Hybrid v4 behaviour, untouched.
"""
...
# Sun avoidance helper (isolated in sun_utils.py for clean diffing)
from sun_utils import sun_cross_mask_tensor
...
# 4P FFA preset — the tuned Hybrid v4 values (kept over the simpler variant).
...
# Adaptive horizon -----------------------------------------------------------
# Early game has few active fleets, so garrison_status simulation is cheap:
# extend the 2P lookahead from 18 to 24 turns at negligible timeout risk.
# 4P games are already expensive AND CONFIG_4P is tuned around horizon=13,
# so 4P never extends.

EARLY_GAME_STEPS = 30     # turns considered "early game"
HORIZON_EARLY    = 24     # extended 2P lookahead
HORIZON_LATE     = 18     # original 2P value


def _adaptive_horizon(step: int, player_count: int) -> int:
    """Return the horizon appropriate for the current game phase and player count."""
    if int(player_count) >= 4:
        return int(CONFIG_4P.horizon)      # never extend in 4P
    return HORIZON_EARLY if int(step) < EARLY_GAME_STEPS else HORIZON_LATE
...
# ---------------------------------------------------------------------------
# cheap_enemy_pressure (unchanged)
# ---------------------------------------------------------------------------

def cheap_enemy_pressure(obs, cache, *, horizon: float, player_id: int) -> Tensor:
    """Cheap reachable-enemy-mass proxy per planet — ``[P]`` (regroup gradient)."""
    P = int(obs.P)
    device = obs.device
...
# ---------------------------------------------------------------------------
# plan_lite_waves — Hybrid v4 pipeline +  sun mask
# ---------------------------------------------------------------------------
...
def plan_lite_waves(
    *,
    ...
    """Single-size, single-source attack planner + regroup (sun-aware)."""
    P = obs.P
    device = obs.device
    dtype = obs.ships.dtype
    ...
    # Sun avoidance — mask out any (source, target) pair whose straight-line
    # path crosses the sun. The one thing the original Producer is missing.
    all_x = obs_tensors["planets"][:, 2].float()         # [P]
    all_y = obs_tensors["planets"][:, 3].float()         # [P]
    src_x = all_x[source_idx.clamp(0, P - 1)]            # [S]
    src_y = all_y[source_idx.clamp(0, P - 1)]            # [S]
    tgt_x = all_x[target_idx.clamp(0, P - 1)]            # [T]
    tgt_y = all_y[target_idx.clamp(0, P - 1)]            # [T]
    crosses_sun = sun_cross_mask_tensor(src_x, src_y, tgt_x, tgt_y)  # [S, T]
    viable = viable & ~crosses_sun                        #  block sun-crossing paths

    # Capture-floor gate at each fleet's arrival turn.
    if K > 0:
    ...
    # FFA leader-guard / prod score bonus (Hybrid v4, 4P+ only)
...
def run_turn(
    ...
    *,
    ...
    """Full per-turn pipeline: adaptive horizon → movement → waves + regroup → emit."""
    ...
    #  Adaptive horizon — patch the frozen config just for this turn.
    # Note: when the horizon changes (step 30 in 2P), ensure_planet_movement
    # rebuilds the movement cache once; this is a one-time acceptable cost.
    horizon = _adaptive_horizon(step, player_count)
...
# ---------------------------------------------------------------------------
# Runtime (Hybrid v4 memory fields + step extraction)
# ---------------------------------------------------------------------------

class ProducerLiteMemory:
    def __init__(self) -> None:
        self.movement: PlanetMovement | None = None
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if step == 0:
            ...
            player_count = mem.cached_player_count

        config = _config_for(player_count)
        row = run_turn(
            obs_tensors, config=config,
            player_count=int(player_count),
            memory=mem, step=step,
        )
